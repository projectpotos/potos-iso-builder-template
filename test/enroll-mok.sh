#!/usr/bin/env bash
#
# The installed system signs its boot chain with a per-host MOK that is
...
LUKS_PASS="$(cfg .disk_encryption.init_password)"
...
if ! sudo test -f "${MNT}/etc/secureboot/mok.der"; then
    ...
    echo "       Is uki.enabled set in input/config.yml and the install complete?" >&2
    exit 1
fi
